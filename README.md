...
Frontend: Vercel
https://email-marketing-app-dun.vercel.app/login

Backend: Render
https://email-marketing-app-cv8z.onrender.com
...
- Account isolation. The assignment emphasized that different users cannot access each other's data. User data isolation (contacts, audiences, campaigns)
...
• Delivered count
• Open tracking
• Automatic analytics refresh (polling)
• Brevo webhook integration

7. ## Known Limitations
- Delivery analytics depend on Brevo Transactional Webhooks.
- During testing, Gmail temporarily rate-limited emails sent from Brevo's shared sending domain, resulting in deferred events instead of delivered events.
- Consequently, deliveredCount remains 0 until a delivered webhook is received from Brevo.
- openedCount was successfully verified using opened and unique_opened webhook events.
...
git clone https://github.com/kaaliyug/email-marketing-app.git
